...
for DATASET in "${DATASETS[@]}"; do
    for SCENE in "${SCENES[@]}"; do
        for N_VIEW in "${N_VIEWS[@]}"; do
            ...
            # Switch colmap to other datasets
            #GT_POSE_PATH=${DATA_ROOT_DIR}/Mipnerf_colmap/${SCENE}/24_views
            #GT_POSE_PATH=${DATA_ROOT_DIR}/MVimgNet_colmap/${SCENE}/24_views
            GT_POSE_PATH=${DATA_ROOT_DIR}/Tanks_colmap/${SCENE}/24_views


            # ----- (1) Mast3r_coarse_geometric_initialization -----
            CMD_D1="CUDA_VISIBLE_DEVICES=${GPU_ID} python ./coarse_init_eval_mast3r.py \
            ...
            # ----- (3) Mast3r_test_pose_initialization -----
            CMD_D2="CUDA_VISIBLE_DEVICES=${GPU_ID} python ./init_test_pose_mast3r.py \
            --img_base_path ${Sparse_image_folder} \
            --n_views ${N_VIEW}  \
            --focal_avg \
            "
            ...
            eval $CMD_T
            echo "========= ${SCENE}: Mast3r_test_pose_initialization ========="
            eval $CMD_D2
            echo "========= ${SCENE}: Render ========="
            eval $CMD_R
            echo "========= ${SCENE}: Metric ========="
